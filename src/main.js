...
// ==========================================
const zonaTactil = document.getElementById("video-wrapper");
let toqueUltimoTiempo = 0;
let temporizadorPresionLarga = null;
let temporizadorAceleracion = null;
let velocidadOriginal = 1;

zonaTactil.addEventListener("touchstart", (e) => {
    // Evitamos ejecutar esto si es un toque con múltiples dedos
    if (e.touches.length > 1) return;

    const tiempoActual = new Date().getTime();
    const tiempoEntreToques = tiempoActual - toqueUltimoTiempo;
    const rectangulo = zonaTactil.getBoundingClientRect();
    const posicionX = e.touches[0].clientX - rectangulo.left;
    const esLadoDerecho = posicionX > rectangulo.width / 2;

    // 1. LÓGICA DE DOBLE TAP (Adelantar / Retroceder)
    if (tiempoEntreToques < 300 && tiempoEntreToques > 0) {
        // Cancelamos el Play/Pause normal al detectar que es un doble toque
        e.preventDefault(); 
        
        if (esLadoDerecho) {
            video.currentTime = Math.min(video.duration, video.currentTime + 10);
            mostrarAvisoVisual("⏩ +10s");
        } else {
            video.currentTime = Math.max(0, video.currentTime - 10);
            mostrarAvisoVisual("⏪ -10s");
        }
        // Reseteamos el tiempo para no registrar un triple tap
        toqueUltimoTiempo = 0; 
        return; 
    }
    
    // 2. LÓGICA DE MANTENER PRESIONADO (Aumentar velocidad gradualmente)
    if (esLadoDerecho) {
        velocidadOriginal = video.playbackRate; // Guardamos la velocidad actual
        
        temporizadorPresionLarga = setTimeout(() => {
            // A los 500ms de presión, cambiamos a 2x
            video.playbackRate = 2.0;
            mostrarAvisoVisual("Velocidad 2x");

            // Si sigue presionando, incrementamos la velocidad cada 1 segundo
            temporizadorAceleracion = setInterval(() => {
                if (video.playbackRate < 4.0) {
                    video.playbackRate += 0.5;
                    mostrarAvisoVisual(`Velocidad ${video.playbackRate}x`);
                }
            }, 1000);
        }, 500); // 500ms es el estándar para "Mantener presionado"
    }

    toqueUltimoTiempo = tiempoActual;
});

zonaTactil.addEventListener("touchend", (e) => {
    // Cancelamos cualquier aceleración que se estuviera preparando o ejecutando
    clearTimeout(temporizadorPresionLarga);
    clearInterval(temporizadorAceleracion);

    // Si soltó el dedo y el video estaba acelerado, volvemos a la normalidad
    if (video.playbackRate !== velocidadOriginal) {
        video.playbackRate = velocidadOriginal;
        ocultarAvisoVisual();
    }
});

zonaTactil.addEventListener("touchcancel", () => {
    clearTimeout(temporizadorPresionLarga);
    clearInterval(temporizadorAceleracion);
    video.playbackRate = velocidadOriginal;
});

// Funciones auxiliares para mostrar texto en pantalla (tipo YouTube/Telegram)
function mostrarAvisoVisual(texto) {
    let aviso = document.getElementById("aviso-gesto");
    if (!aviso) {
        aviso = document.createElement("div");
        aviso.id = "aviso-gesto";
        aviso.style.cssText = "position:absolute; top:20px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,0.6); color:white; padding:8px 16px; border-radius:20px; z-index:100; pointer-events:none; font-weight:bold; transition: opacity 0.2s;";
        zonaTactil.appendChild(aviso);
    }
    aviso.textContent = texto;
    aviso.style.opacity = "1";
}

function ocultarAvisoVisual() {
    let aviso = document.getElementById("aviso-gesto");
